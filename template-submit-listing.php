...
$ct_front_submit_condominio = isset( $ct_options['condominio'] ) ? esc_html( $ct_options['condominio'] ) : '';
$ct_front_submit_iptu = isset( $ct_options['iptu'] ) ? esc_html( $ct_options['iptu'] ) : '';
        ...
        jQuery("form input").on("keypress", function(e) {
            ...
            <?php if(!is_user_logged_in()) {
            	...
            	echo '<div class="must-be-logged-in">';
					echo '<h4 class="center marB20">' . __('Você deve estar logado para visualizar esta página.', 'contempo') . '</h4>';
                ...
                if(function_exists('ct_create_packages') && $item_count == 0 && empty($post_id)) {
                	...
                	echo '<div class="col span_12 first packages-notification-large">';
	                    echo '<h4 class="marB20">' . __( 'Você ainda não escolheu um pacote', 'contempo' ) . '</h4>';
	                    echo '<p class="marB0"><a class="btn" href="' . get_page_link($ct_package_list) . '">' . __('Iniciar!', 'contempo') . '</a></p>';
	                echo '</div>';

	            } elseif(function_exists('ct_create_packages') && $item_count >= $listing_included) {
                	
                	echo '<div class="col span_12 first packages-notification-large">';
	                    echo '<h4 class="marB5">' . __( 'Limite de anúncios atingido', 'contempo' ) . '</h4>';
	                    echo '<p class="marB20">' . __('Você atingiu o limite de anúncios para o seu pacote de associação.', 'contempo') . '</p>';
	                    echo '<p class="marB0"><a class="btn" href="' . get_page_link($ct_package_list) . '">' . __('Upgrade hoje!', 'contempo') . '</a></p>';
	                echo '</div>';
        			...
        			<form action="" id="primaryPostForm" class="front-end-form" method="POST" enctype="multipart/form-data">
                        ...
                        <fieldset class="col span_10 first form-section">
                            ...
                            <div class="col span_6 first">
                                <label><?php esc_html_e('Endereço', 'contempo'); ?></label>
                                <input type="text" name="postTitle" id="postTitle" value="<?php if ( isset( $_POST['postTitle'] ) ) echo esc_attr($_POST['postTitle']); ?>" placeholder="1234 Rua das Acácias" <?php if($ct_front_submit_street_address == 'required') { echo 'required'; } ?> />
                            </div>

                            <div class="col span_6">
                                <label><?php esc_html_e('Título do Anúncio', 'contempo'); ?></label>
                                <input type="text" name="customMetaAltTitle" id="customMetaAltTitle" value="<?php if ( isset( $_POST['customMetaAltTitle'] ) ) echo esc_attr($_POST['customMetaAltTitle']); ?>" placeholder="<?php esc_html_e('(ex. Mansão no Condomínio Laranjeiras)', 'contempo'); ?>" <?php if($ct_front_submit_alt_title == 'required') { echo 'required'; } ?> />
                            </div>

                                <div class="clear"></div>
                                
                            <div id="status" class="col span_4 first">
                                <label><?php esc_html_e('Status', 'contempo'); ?></label>
                                <?php ct_submit_listing_form_select_show_all('ct_status'); ?>
                            </div>

                            <div id="property_type" class="col span_4">
                                <label><?php esc_html_e('Tipo', 'contempo'); ?></label>
                                <?php ct_submit_listing_form_select_show_all('property_type'); ?>
                            </div>
	                            ...
	                            <?php if(function_exists('ct_create_packages')) { ?>
									...
									<?php } else { ?>
										<select id="ct_featured_status" name="ct_featured_status" disabled="">
											<option value=""><?php if($featured_listings_included == 0) { _e('Não incluso', 'contempo'); } elseif($featured_listings_included >= 1) { _e('Limite do pacote atingido', 'contempo'); } ?></option>
										</select>
									<?php } ?>
								<?php } else { ?>
									<select id="ct_featured_status" name="ct_featured_status">
										<option value=""><?php _e('Não', 'contempo'); ?></option>
										<option value="featured"><?php _e('Yes', 'contempo'); ?></option>
									</select>
								<?php } ?>
                            </div>
                            ...
                            <div class="col span_4 marT15">
                                <label><?php esc_html_e('Prefixo', 'contempo'); ?></label>
                                <input type="text" name="customMetaPricePrefix" id="customMetaPricePrefix" placeholder="<?php esc_html_e(' (ex. Ligue para consultar)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaPricePrefix'])) echo esc_attr($_POST['customMetaPricePrefix']);?>" <?php if($ct_front_submit_price_prefix == 'required') { echo 'required'; } ?> />
                            </div>

                            <div class="col span_4 marT15">
                                <label><?php esc_html_e('Texto após Preço', 'contempo'); ?></label>
                                <input type="text" name="customMetaPricePostfix" id="customMetaPricePostfix" placeholder="<?php esc_html_e(' (ex. /Semana /Mês)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaPricePostfix'])) echo esc_attr($_POST['customMetaPricePostfix']);?>" <?php if($ct_front_submit_price_postfix == 'required') { echo 'required'; } ?> />
                            </div>

                                <div class="clear"></div>

                            <label><?php esc_html_e('Descrição do Anúncio ', 'contempo'); ?></label>
                            <?php
                                $content = '';
                        ...
                        <fieldset class="col span_10 first form-section">
                            ...
                            <div style="display: none;">
                                <div class="left">
                                    <label><?php esc_html_e('Imagem em destaque do anúncio', 'contempo'); ?></label>
                                    <input type="file" name="featuredImage" id="featuredImage" />
                                </div>

                                <div class="left">
                                    <label><?php esc_html_e('Galeria de Imagens (selecione quantos quiser)', 'contempo'); ?></label>
                            ...
                            <div class="1-left">
                                <div class="col span_12 first">
                                    <label><?php esc_html_e('Imagens', 'contempo'); ?></label>
                                    ...
                                    <div class="drag-drop col span_12 first row">
                                        <div id="drag-drop-area" class="drag-drop-area">
                                            <div class="drag-drop-msg">
                                                ...
                                                <strong><?php esc_html_e('Arraste e solte imagens aqui', 'contempo'); ?></strong>
                                            </div>
                                            <div class="drag-drop-or">
                                                <?php esc_html_e('ou', 'contempo'); ?>
                                            </div>
                                            <div class="drag-drop-btn">
                                                <a id="select-images" class="btn" href="javascript:;"><?php esc_html_e('Selecionar Imagens', 'contempo'); ?></a>
                                            </div>
                                        </div>
                                        <input style="display: none;" type="file" name="galleryImages" id="galleryImages" multiple="" />
                                        <p class="muted marT10 marB0"><?php esc_html_e('*Pelo menos uma imagem é para envio válido, largura mínima de 817 px.', 'contempo'); ?></p>
                                        <p class="muted marB0"><?php esc_html_e('*Você pode marcar uma imagem como destaque clicando no ícone de estrela. Caso contrário, a primeira imagem será considerada imagem em destaque.', 'contempo'); ?></p>
							...
							<div class="1-left">
								<div id="files-documents" class="col span_12 first">
                                    <label class="left"><?php esc_html_e('Arquivos e Documentos', 'contempo'); ?></label>
									<input type="button" id="select-file" class="btn right" value="Adicionar ou fazer upload de arquivos" />
										<div class="clear"></div>
									...
									<p class="muted marB0 small"><?php esc_html_e('*Os tipos de arquivos suportados são PDF, Word, Excel e PowerPoint.', 'contempo'); ?></p>
								</div>
							</div>
                        </fieldset>
                        ...
                        <fieldset class="col span_10 first form-section">
                            ...
                            <!-- <div class="col span_4">
                                <label><?php esc_html_e('Animais de estimação', 'contempo'); ?></label>
                                <input type="text" name="customMetaPets" id="customMetaPets" value="<?php if(isset($_POST['customMetaPets'])) echo esc_attr($_POST['customMetaPets']);?>" placeholder="<?php _e('(exe. Gatos, cães pequenos)', 'contempo'); ?>" <?php if($ct_front_submit_pets == 'required') { echo 'required'; } ?> />
                            </div> -->

                            <div class="col span_4 first">
                                <label><?php esc_html_e('Estacionamento', 'contempo'); ?></label>
                                <input type="text" name="customMetaParking" id="customMetaParking" value="<?php if(isset($_POST['customMetaParking'])) echo esc_attr($_POST['customMetaParking']);?>" placeholder="<?php _e('(ex. Garagem, garagem de 2 carros)', 'contempo'); ?>" <?php if($ct_front_submit_parking == 'required') { echo 'required'; } ?> />
                            </div>

                            <div class="col span_6 ">
                                <label><?php esc_html_e('ID Propriedade ', 'contempo'); ?></label>
                            ...
                            <div class="col span_12 first">
                                <label><?php esc_html_e('URL Tour Virtual', 'contempo'); ?></label>
                            ...
                            <div id="listing-open-house">
                                ...
                                <h5 class="marT0 border-bottom"><?php _e('Casa aberta para visitação', 'contempo'); ?></h5>

                                <div class="col span_4 first">
                                    <label><?php esc_html_e('Data', 'contempo'); ?></label>
                                ...
                                <div class="col span_4">
                                    <label><?php esc_html_e('Início', 'contempo'); ?></label>
                                ...
                                <div class="col span_4">
                                    <label><?php esc_html_e('Fim', 'contempo'); ?></label>
                            ...
                            <label><?php esc_html_e('Características adicionais (separado por vírgula)', 'contempo'); ?></label>
                            <textarea name="customTaxFeat" id="customTaxFeat" rows="8" cols="30" placeholder="Piscina, Spa, Condomínio Fechado" <?php if($ct_front_submit_additional_features == 'required') { echo 'required'; } ?>><?php ct_post_additional_features(); ?></textarea>
                        ...
                        if($ct_rentals_booking == 'yes' || class_exists('Booking_Calendar') && $ct_submit_rental_info == 'yes') { ?>
                            ...
                            <fieldset class="col span_10 first form-section">
                                ...
                                <div class="col span_6 first">
                                    <label><?php esc_html_e('Número máximo de convidados', 'contempo'); ?></label>
                                    <input type="number" name="customMetaMaxGuests" id="customMetaMaxGuests" placeholder="<?php esc_html_e(' (exe. 2)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaMaxGuests'])) echo esc_attr($_POST['customMetaMaxGuests']);?>" <?php if($ct_front_submit_max_guests == 'required') { echo 'required'; } ?> />
                                </div>

                                <div class="col span_6">
                                    <label><?php esc_html_e('Estadia mínima', 'contempo'); ?></label>
                                    <input type="number" name="customMetaMinStay" id="customMetaMinStay" placeholder="<?php esc_html_e(' (exe. 1 night)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaMinStay'])) echo esc_attr($_POST['customMetaMinStay']);?>" <?php if($ct_front_submit_min_stay == 'required') { echo 'required'; } ?> />
                                </div>

                                    <div class="clear"></div>

                                <div class="col span_6 first">
                                    <label><?php esc_html_e('Hora do Check-in', 'contempo'); ?></label>
                                    <input type="number" name="customMetaCheckIn" id="customMetaCheckIn" placeholder="<?php esc_html_e(' (exe. 3:00 PM)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaCheckIn'])) echo esc_attr($_POST['customMetaCheckIn']);?>" <?php if($ct_front_submit_check_in == 'required') { echo 'required'; } ?> />
                                </div>

                                <div class="col span_6">
                                    <label><?php esc_html_e('Hora do Check Out', 'contempo'); ?></label>
                                    <input type="number" name="customMetaCheckOut" id="customMetaCheckOut" placeholder="<?php esc_html_e(' (exe. 11:00 AM)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaCheckOut'])) echo esc_attr($_POST['customMetaCheckOut']);?>" <?php if($ct_front_submit_check_out == 'required') { echo 'required'; } ?> />
                                </div>

                                    <div class="clear"></div>

                                <div class="col span_6 first">
                                    <label><?php esc_html_e('Taxa para Pessoa Extra', 'contempo'); ?> (<?php ct_currency(); ?>)</label>
                                    <input type="number" name="customMetaExtraPerson" id="customMetaExtraPerson" placeholder="<?php esc_html_e(' (exe. 50)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaExtraPerson'])) echo esc_attr($_POST['customMetaExtraPerson']);?>" <?php if($ct_front_submit_extra_person == 'required') { echo 'required'; } ?> />
                                </div>

                                <div class="col span_6">
                                    <label><?php esc_html_e('Taxa de limpeza', 'contempo'); ?> (<?php ct_currency(); ?>)</label>
                                    <input type="number" name="customMetaCleaningFee" id="customMetaCleaningFee" placeholder="<?php esc_html_e(' (exe. 150)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaCleaningFee'])) echo esc_attr($_POST['customMetaCleaningFee']);?>" <?php if($ct_front_submit_cleaning_fee == 'required') { echo 'required'; } ?> />
                                </div>

                                    <div class="clear"></div>

                                <div class="col span_6 first">
                                    <label><?php esc_html_e('Taxa de cancelamento', 'contempo'); ?> (<?php ct_currency(); ?>)</label>
                                    <input type="number" name="customMetaCancellationFee" id="customMetaCancellationFee" placeholder="<?php esc_html_e(' (exe. 275)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaCancellationFee'])) echo esc_attr($_POST['customMetaCancellationFee']);?>" <?php if($ct_front_submit_cancellation_fee == 'required') { echo 'required'; } ?> />
                                </div>

                                <div class="col span_6">
                                    <label><?php esc_html_e('Depósito de segurança', 'contempo'); ?> (<?php ct_currency(); ?>)</label>
                                    <input type="number" name="customMetaSecurityDeposit" id="customMetaSecurityDeposit" placeholder="<?php esc_html_e(' (exe. 895)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaSecurityDeposit'])) echo esc_attr($_POST['customMetaSecurityDeposit']);?>" <?php if($ct_front_submit_security_deposit == 'required') { echo 'required'; } ?> />
                                </div>
                        ...
                        <fieldset class="col span_10 first form-section">
                            ...
                            <div class="input-full-width">
                                <label><?php esc_html_e('Endereço', 'contempo'); ?></label>
                                <input type="text" name="pac-input" id="pac-input" value="" placeholder="<?php _e('Digite um endereço', 'contempo'); ?>" <?php if($ct_front_submit_address == 'required') { echo 'required'; } ?> />
                            ...
                            <div class="col span_4 first">
                                <label><?php esc_html_e('Município', 'contempo'); ?></label>
                            ...
                            <div class="col span_12 first">
                                <input type="text" name="customMetaLatLng" id="customMetaLatLng" placeholder="<?php esc_html_e('Latitude e Longitude (opcional)', 'contempo'); ?>" value="<?php if(isset($_POST['customMetaLatLng'])) echo esc_attr($_POST['customMetaLatLng']);?>" <?php if($ct_front_submit_lat_long == 'required') { echo 'required'; } ?> />
                            </div>
                            ...
                            <p class="form-note"><?php _e('Você também pode arrastar manualmente o marcador para o local exato da sua listagem se a geolocalização automática estiver desativada.', 'contempo'); ?></p>
                        ...
                        <fieldset class="col span_10 first form-section">
                            ...
                            <label><?php esc_html_e('Notas particulares', 'contempo'); ?></label>
                            <textarea name="customOwnerNotes" id="customOwnerNotes" rows="8" cols="30" placeholder="<?php _e('Escreva uma nota particular sobre este anúncio, pois a área de texto não será exibida em nenhum lugar do front end do site.', 'contempo'); ?>" <?php if($ct_front_submit_private_notes == 'required') { echo 'required'; } ?>><?php ct_post_private_notes(); ?></textarea>
                        ...
                        <div class="col span_12 first fieldset-buttons">
                            <div class="col span_9 first">
                                <a class="btn btn-cancel left" href="<?php echo get_page_link($user_listings); ?>" data-tooltip="<?php _e('Cancelar', 'contempo'); ?>"><i class="fa fa-close"></i></a>
                                <input class="btn save-draft btn-secondary left" href="#" type="submit" value="<?php esc_html_e('Salvar como rascunho', 'contempo'); ?> " name="draft" style="display:block !important"/>
                            </div>
							<div style="display: none;">
                            ...
                            <div class="col span_3">
                                <a name="next" class="next btn right" data-tooltip="<?php _e('Proximo Passo', 'contempo'); ?>"><i class="fa fa-chevron-right"></i></a>
                                ...
                                <a name="previous" class="previous btn right" data-tooltip="<?php _e('Anterior', 'contempo'); ?>"><i class="fa fa-chevron-left"></i></a>
                            </button>
                        </div>
            ...
            <?php wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Páginas:', 'contempo' ) . '</span>', 'after' => '</div>' ) ); ?>
