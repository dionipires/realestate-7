 ...
 *
 *
 * Template Name: Demo Home Listings Slider
 *
...
get_header();
	...
	echo 'jQuery(window).load(function() {';
		echo 'jQuery("#header-wrap").addClass("trans-header");';
	echo '});';
	echo '</script>';

	echo '<style>';
	echo '.tp-bgimg.defaultimg { opacity: 0.4 !important;}';
	echo '#main-content { position: relative; top: -149px;}';
	echo '</style>';

	        echo '<!-- Revolution Slider -->';
            echo '<section class="rev-slider">';
            if(class_exists('RevSlider')) {
                putRevSlider('listing-showcase');
            }
            echo '</section>';
            echo '<!-- //Revolution Slider -->';
